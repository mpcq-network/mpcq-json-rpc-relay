...
describe('OFTAdapterTests', function() {
  it('@hedera @fund-and-approve transfer to adapter', async () => {
    const contractERC20 = await ethers.getContractAt('ERC20Mock', process.env.ERC20_MPCQNET_CONTRACT);
    const transferTx = await contractERC20.transfer(process.env.OFT_ADAPTER_MPCQNET_CONTRACT, amount);
  ...
  it('@hedera @fund-and-approve adapter approval', async () => {
    const contractERC20 = await ethers.getContractAt('ERC20Mock', process.env.ERC20_MPCQNET_CONTRACT);
    const approveTx = await contractERC20.approve(process.env.OFT_ADAPTER_MPCQNET_CONTRACT, amount);
  ...
  it('@hedera @send to bsc', async () => {
    ...
    const contract = await ethers.getContractAt('ExampleOFTAdapter', process.env.OFT_ADAPTER_MPCQNET_CONTRACT);
  ...
  it('@hedera @test balance', async () => {
    ...
    console.log(`(${hre.network.name}) oft adapter balance: ${await contractERC20.balanceOf(process.env.OFT_ADAPTER_MPCQNET_CONTRACT)}`);

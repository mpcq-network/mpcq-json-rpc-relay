...
describe('ONFTAdapterTests', function() {
  it('@hedera @mint', async () => {
    ...
    const contract = await ethers.getContractAt('ERC721Mock', process.env.ERC721_MPCQNET_CONTRACT);
    ...
    const txAdapter = await contract.mint(process.env.ONFT_ADAPTER_MPCQNET_CONTRACT, 2);
  ...
  it('@hedera @approve adapter', async () => {
    const contract = await ethers.getContractAt('ERC721Mock', process.env.ERC721_MPCQNET_CONTRACT);
    const approveTx = await contract.approve(process.env.ONFT_ADAPTER_MPCQNET_CONTRACT, 1);
  ...
  it('@hedera @send to bsc', async () => {
    ...
    const contract = await ethers.getContractAt('ExampleONFTAdapter', process.env.ONFT_ADAPTER_MPCQNET_CONTRACT);

...
describe('HTSConnectorTests', function() {
  it('@hedera @approve oft hts contract', async() => {
    const oftHts = await ethers.getContractAt('ExampleHTSConnector', process.env.HTS_CONNECTOR_MPCQNET_CONTRACT);
    const tokenAddress = await oftHts.htsTokenAddress();

    const contract = await ethers.getContractAt('ERC20', tokenAddress);
    const txApprove = await contract.approve(process.env.HTS_CONNECTOR_MPCQNET_CONTRACT, amount);
  ...
  it('@hedera @send to bsc', async () => {
    ...
    const contract = await ethers.getContractAt('ExampleHTSConnector', process.env.HTS_CONNECTOR_MPCQNET_CONTRACT);
  ...
  it('@hedera @test balance', async () => {
    ...
    console.log(`(${hre.network.name}) oft contract balance: ${await contract.balanceOf(process.env.HTS_CONNECTOR_MPCQNET_CONTRACT)}`);
